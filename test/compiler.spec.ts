...
async function getScript(fileName: string): Promise<{fileName: string, content: string}> {
...
describe('Compiler', async function(){    
    ...
    it("getDependencies", async ()=>{
        ...
        let deps = await compiler.getDependencies('form.tsx', (await getScript('form.tsx')).content);        
        assert.deepStrictEqual(deps, ['@ijstech/components', 'hello'])
    });
    it("getDependencies with FileImporter", async ()=>{
        let compiler = new Compiler();
        let deps = await compiler.getDependencies('form.tsx', (await getScript('form.tsx')).content, getScript);        
        assert.deepStrictEqual(deps, [ '@ijstech/components', 'hello.ts', 'test1.ts', 'lib/test2.ts' ])
    });
    it('hello.ts', async ()=>{
        let compiler = new Compiler();
        await compiler.addFile('hello.ts', (await getScript('hello.ts')).content, getScript);        
        ...
        if (exp)
            ...
            await setScriptContent('hello.js', result.script['index.js']);
    });    
    it('form.tsx', async ()=>{        
        let compiler = new Compiler();
        await compiler.addFile('form.tsx', (await getScript('form.tsx')).content, getScript);
        let result = await compiler.compile(true);        
        if (result.errors.length)
            console.dir(result.errors)
        assert.strictEqual(result.errors.length, 0);
        ...
        if (exp)
            ...
            await setScriptContent('form.js', result.script['index.js']);
    });
});