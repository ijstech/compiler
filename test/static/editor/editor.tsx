        ...
        return (
            <i-panel id='pnlMain' dock='fill'>
                <i-panel height={40} dock='top'>
                    <i-panel dock='left'>
                        <i-button caption="Run" icon="caret-right" height={30} width={140} margin={{top: 5, left: 4}}
                                  onClick={this.run}></i-button>
                    </i-panel>
                    <i-panel dock='right' width={140} padding={{top: 4, bottom: 4}}>
                        <i-hstack>
                            <i-label caption='Preview' width={60}></i-label>
                            <i-switch checkedText='on' uncheckedText='off' id='swPreview' width={80} checked={true}
                                      onChanged={this.handleTogglePreview}></i-switch>
                        </i-hstack>
                    </i-panel>
                </i-panel>
                <i-panel dock='left' width={180} resizer={true}>
                    <i-tabs mode='vertical' dock='fill' width={80}>
                        <i-tab icon={{name: 'file-code', fill: 'white'}}>
                            <i-tree-view id='tvFiles' dock='fill' onClick={this.handleTreeViewClick}
                                         onDblClick={this.handleTreeViewDblClick}></i-tree-view>
                        </i-tab>
                ...
                <i-panel id='pnlCode' dock='fill'>
                    <i-tabs id='tsEditors' dock='fill' draggable={true} closable={true}
                            onTabClosed={this.handleEditorTabClose}>
                        <i-tab id='tabCodeTemp' caption='untitled'>
                            <i-code-editor id="edtCodeTemp" dock='fill'
                                           onChange={this.handleFileChange}></i-code-editor>
                        </i-tab>
                    </i-tabs>
                </i-panel>
                <i-panel id='pnlPreview' dock='right' width='50%' resizer={true}>
                    <i-panel dock='top' height={30} padding={{top: 5, bottom: 5}}>
                        <i-panel dock='left' width={80}>
                            ...
                            <i-button icon='redo' margin={{left: 4}} width={20} height={20}
                                      onClick={this.reload}></i-button>
                        </i-panel>
