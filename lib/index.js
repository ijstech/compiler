...
exports.Compiler = exports.PackageManager = exports.resolveAbsolutePath = exports.bundleDapp = exports.bundleWidget = exports.bundleWorker = exports.bundleContractDist = exports.bundleContractLib = exports.bundleContract = exports.EPackageType = exports.Types = exports.Path = exports.Parser = void 0;
...
;
async function bundleContractLib(storage, RootPath) {
    RootPath = RootPath || storage.rootPath;
...
exports.bundleContractLib = bundleContractLib;
;
async function bundleContractDist(storage, RootPath) {
    RootPath = RootPath || storage.rootPath;
    let packageConfig = await storage.getPackageConfig();
    if (packageConfig) {
        let packageManager = new PackageManager({
...
exports.bundleContractDist = bundleContractDist;
;
...
async function bundleDapp(storage, RootPath) {
    ...
    if (scconfig) {
        ...
        async function copyDependencies(dependencies, all) {
            ...
            for (let name in dependencies) {
                ...
                if ((all || name.startsWith('@ijstech/') || name.startsWith('@scom/'))) {
                    let pack = await storage.copyPackage(name, path_1.default.join(distLibDir, name));
                    if (pack) {
                        deps.unshift(name);
        ...
        async function bundleDependencies(dependencies) {
            ...
            for (let name in dependencies) {
                ...
                if (scconfig.modules[name]) {
                    content = await storage.readFile(path_1.default.join(distModuleDir, `${scconfig.modules[name].path}/index.js`));
                }
                else
                    content = await storage.readFile(path_1.default.join(distLibDir, name, 'index.js'));
                if (content)
        ...
        if (scconfig.bundle) {
            ...
            ;
            await storage.writeFile(path_1.default.join(distDir, scconfig.rootDir || '', 'bundle.json'), JSON.stringify(bundleLibs));
        }
        else
            await copyDependencies(scconfig.dependencies, true);
        if (scconfig.packages) {
            scconfig.packages.forEach(async (name) => {
                await storage.copyPackage(name, path_1.default.join(distLibDir, name));
            });
        ...
        if (scconfig.ipfs == true) {
            ...
            await storage.writeFile(path_1.default.join(distDir, 'ipfs.json'), JSON.stringify(cid));
            ...
            indexHtml = indexHtml.replace('{{main}}', `${scconfig.main || '@scom/dapp'}`);
            await storage.writeFile(path_1.default.join(distDir, 'scconfig.json'), JSON.stringify(scconfig, null, 4));
            await storage.writeFile(path_1.default.join(scRootDir, scconfig.distDir || 'dist', 'index.html'), indexHtml);
        }
    }
    ;
}
