...
exports.Compiler = exports.PackageManager = exports.resolveAbsolutePath = exports.bundleDapp = exports.bundleWidget = exports.bundleWorker = exports.bundleDist = exports.bundleLib = exports.bundleSdk = exports.bundleTactContract = exports.bundleContract = exports.EPackageType = exports.Types = exports.Path = exports.Parser = void 0;
...
exports.Types = Types;
const tactCompile_1 = require("./tactCompile");
let isNode = false;
...
;
async function bundleTactContract(storage, RootPath, config) {
    let options = config;
    if (!options) {
        RootPath = RootPath || storage.rootPath;
    ...
    const files = await (0, tactCompile_1.compileTactContract)(storage, options);
    if (files && !config) {
        let promises = [];
...
exports.bundleTactContract = bundleTactContract;
;
...
;
async function bundleLib(bundleType, storage, RootPath) {
    RootPath = RootPath || storage.rootPath;
    ...
    if (packageConfig) {
        ...
        if (scconfig) {
            ...
            for (let name in scconfig.modules) {
                let path = path_1.default.join(moduleSourceDir, scconfig.modules[name].path);
        ...
        ;
        let pack = { files: await storage.getFiles(path_1.default.join(scRootDir, srcDir)) };
        for (let n in pack.files) {
            if (n == 'index.ts' || n == 'index.tsx')
                pack.files[n] = `///<amd-module name='${packageConfig.name}'/> \n` + pack.files[n];
            else
        ...
        if (scconfig?.bundleLibs?.before) {
            for (let i = 0; i < scconfig.bundleLibs.before.length; i++) {
                ...
                let fullPath = path_1.default.join(scRootDir, libPath);
        ...
        ;
        await storage.copyAssets(path_1.default.join(scRootDir, srcDir), distDir);
        await storage.writeFile(path_1.default.join(distDir, 'index.js'), script);
        await storage.writeFile(path_1.default.join(distDir, 'scconfig.json'), JSON.stringify({
            name: packageConfig.name,
            type: "widget",
...
if (!rootDir.endsWith('/'))
    ...
    if (scconfig) {
        ...
        function createPackageManager(tsconfig) {
        ...
        async function addCommonPackages(pm) {
        ...
        let reactPackages = [];
        let customWebComponentPackages = [];
        for (let name in packages) {
            let isReactPackage = false;
        ...
        async function addBuiltPackagesToManager(names, fromManager, toManager) {
        ...
        ;
        let deps = [];
        await storage.copyPackage('@ijstech/components', path_1.default.join(distLibDir, '@ijstech/components'), scconfig.packages);
                ...
                if (name != '@ijstech/components/' && !name.startsWith('@ijstech/components/')) {
                    if ((all || scconfig.modules[name] || name.startsWith('@ijstech/') || name.startsWith('@scom/') || name.startsWith('@trustroot/'))) {
                        ...
                        if (scconfig.modules[name])
                            ...
                            pack = await storage.copyPackage(name, path_1.default.join(distLibDir, name), scconfig.packages);
                        if (pack) {
        ...
        if (scconfig.manifest) {
            manifestLink = `  <link rel="manifest" href="${scconfig.version ? '/' + scconfig.version : ''}/manifest.webmanifest" >`;
            let manifest = {
                "scope": "/",
                "start_url": "/",
                "display": "standalone",
            ...
            await storage.writeFile(path_1.default.join(distDir, scconfig.version, 'manifest.webmanifest'), JSON.stringify(manifest, null, 4));
        }
...
class PackageManager {
    ...
    ;
    async buildPackage(name, storage) {
        let pack = this._packages[name];
        if (!pack.dts && pack.files) {
            if (storage?.onCompile)
                storage.onCompile(name);
            let indexFile = '';
            if (pack.indexFile)
...
class Compiler {
    ...
    ;
    locateControl(fileName, control) {
        const source = this.getSource(fileName);
        if (source) {
