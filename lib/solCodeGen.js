...
function default_1(name, abiPath, abi, linkReferences, options) {
    ...
    const toSolidityInput = function (item) {
        let prefix = item.inputs && item.inputs.length > 1 ? "params." : "";
        let result = toSolidityType(prefix, item.inputs);
        return result;
    };
    const payable = function (item) {
        return (item.inputs && item.inputs.length == 0 ? '' : ', ') +
    ...
    const callFunction = function (name, item) {
        ...
        else if (item.inputs && item.inputs.length == 1) {
        ...
        returnOutputs(item.outputs || [], true, false).forEach((e, i, a) => addLine(e.indent + 3, e.text || ''));
        addLine(2, '}');
    };
    ...
    const addDeployer = function (abi, linkReferences) {
